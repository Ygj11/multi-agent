...
def test_create_app_uses_disabled_knowledge_service_by_default(app_factory):
    ...
    assert isinstance(app.state.container.knowledge_service, DisabledKnowledgeService)
...
def test_create_app_can_use_knowledge_api_client_when_factory_returns_it(app_factory, monkeypatch):
    ...
    monkeypatch.setattr("app.bootstrap.container.build_knowledge_service", lambda settings: service)
    app = app_factory("knowledge-enabled.sqlite3")

    assert isinstance(app.state.container.knowledge_service, KnowledgeAPIClient)
