...
async def test_pipeline_checks_required_arguments_before_write_approval():
    ...
    assert result.error == "missing_required_argument:status"
...
async def test_pipeline_checks_authorization_before_write_approval():
    registry = _registry()
    registry.get_definition("update_policy_status").required_scopes = ["policy:write"]
    ...
    result = await executor.execute(
        ...
        agent_card=_card("policy_query_agent"),
        tool_name="update_policy_status",
        arguments={"policy_no": "P001", "status": "cancelled"},
        principal=principal,
    )
...
async def test_pipeline_routes_write_tool_to_human_approval_after_guards_pass():
    ...
    assert result.approval_payload["tool_name"] == "update_policy_status"
    assert result.pending_tool_call["arguments"] == {"policy_no": "P001", "status": "cancelled"}
