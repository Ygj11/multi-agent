...
from app.schemas.enums.tool import DataClassification, RiskLevel, ToolOperation, ToolScope, ToolSource
...
class ToolDefinition(BaseModel):
    ...
    scope: ToolScope = Field(default=ToolScope.PUBLIC, description="工具可见性范围：public 公共工具，private 某 Agent 私有工具，mcp 动态发现工具。")
    source: ToolSource = Field(default=ToolSource.LOCAL, description="执行来源：local 调本地 callable；mcp 通过 MCP client 调外部 server。")
    ...
    operation: ToolOperation = Field(default=ToolOperation.READ, description="操作类型，影响审批、资源访问 action 和审计；如 read/write/notify/delete/ddl。")
    ...
    data_classification: DataClassification = Field(default=DataClassification.INTERNAL, description="工具结果数据分级：public/internal/confidential/sensitive。")
    risk_level: RiskLevel = Field(default=RiskLevel.LOW, description="工具风险等级；high 默认需要审批。")
