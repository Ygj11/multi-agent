...
class PostgreSQLCheckpointBackendExample:
    ...
    async def save(self, thread_id: str, snapshot: dict[str, Any], request_id: str | None = None, trace_id: str | None = None) -> None:
        if not self.dsn:
            raise RuntimeError("PostgreSQL checkpoint DSN is not configured; real backend is disabled by default.")
        # TODO: INSERT snapshot_json ... ON CONFLICT(thread_id) DO UPDATE。

    async def load(self, thread_id: str, request_id: str | None = None, trace_id: str | None = None) -> dict[str, Any] | None:
        if not self.dsn:
            raise RuntimeError("PostgreSQL checkpoint DSN is not configured; real backend is disabled by default.")
        # TODO: SELECT snapshot_json FROM checkpoints WHERE thread_id = ...
        return None
