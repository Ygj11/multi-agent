...
当前 troubleshooting 有 mock handler `mvp_agent_tool_handlers.py` 与 real factory `troubleshooting_real_tool_handlers.py`；POS 使用 `pos_query_mock_client.py` 与 `pos_query_tool_handlers.py`。新增业务工具时先确定它是否需要同名 mock/real 两种实现，再保持注册名稳定。若新增真实领域 client，先扩展 `app/integrations/clients.py:IntegrationClients`，再由 `app/bootstrap/container.py:_build_integration_clients()` 装配；`register_agent_private_tools()` 只接收该集合，不再增加单个 client 参数。
